...
public class SocialNetwork {
  ...
  public void addFriendshipRequest(String m1, String m2) {
    int requester = -1, requested = -1;
    for (int i = 0; i < members.length; i++) {
      if(members[i]==null) break;
      if(m1.equals(members[i].name())) requester = i;
      if(m2.equals(members[i].name())) requested = i;
    }
    if(requests.length==numRequests){
      FriendshipRequest newRequests[] = new FriendshipRequest[numRequests+sizeExpansion];
      System.arraycopy(requests,0,newRequests,0,numRequests);
      requests = newRequests;
    }
    requests[numRequests] = new FriendshipRequest(requester, requested);
    numRequests++;
  }
  ...
  public int numPendingRequests(String memberName) {
    int index = memberIndex(memberName);
    int count = 0;
    for (int i = 0; i < requests.length; i++) {
      if(requests[i]==null) break;
      if(requests[i].requester()==index) {
  ...
  public String oldestFriend(String memberName) {
    ...
    for (int i = 0; i < requests.length; i++) {
      ...
      int friendIndex = -1;
      if(requests[i].requested()==index) friendIndex = requests[i].requester();
      if(requests[i].requester()==index) friendIndex = requests[i].requested();

      Person friend = members[friendIndex];
      if(min>friend.birthYear()){
        min = friend.birthYear();
        name = friend.name();
      }
    }
    return name;
